# Minexcoin CPU, CUDA & OpenCL Miner v0.17a by Racquemis

Binaries are available for Windows And Linux. 64 bit only.
-
For Windows is available the version 0.20.1 of MxMiner. Information in the changelog file.
```
...
Make sure you installed the latest version of 'Visual C++ 2013 Redistributable x64'. You can download it from the microsoft website.
...
Miner works best at 75% CPU utilization. Use Number of CPU Threads Minus 1. You also remove -t parameter for the miner to select the number of Threads automatically.
...
Write down the platform number and the device numbers of each device you want to mine with you can only mine with devices from a single platform. if you want to mine devices of multiple platforms you must start a seperate instance for each platform.

Set the -op parameter to the platform number next set the -od parameter with the device numbers. If you want to mine with device #0 and device #1 you type: -od 0 1. Optionally you can  set the number of threads for each device with the -ot command -ot 1 1 sets the threads for both devices to one. This means at a single instance of the solver will run on the card. If you see that your GPU isn't loaded fully set -ot 2 2 and try again.
...
Grant permission to the mxminer with the command chmod +x before you start mining. Otherwise the permission to run the mxminer will be denied.
